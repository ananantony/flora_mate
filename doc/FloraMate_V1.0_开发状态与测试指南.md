...
| **继电器** | `bsp_relay.c/h` | 6 路继电器（CH1–CH4 阀 + CH5 主阀 + CH6 预留）；`Selftest`；`MainOffForce` 紧急全断 | ✅ 完成 |
| **水泵 PWM** | `bsp_pump_pwm.c/h` | TIM2-CH1（PA0）1 kHz，1000 级分辨率；`SetDutyPercent` / `Stop` | ✅ 完成 |
| **按键** | `bsp_key.c/h` | 4 键（PA1–PA4）消抖 20 ms；短按 / 长按 / 超长按阈值可配；事件回调注入 | ✅ 完成 |
| **OLED** | `bsp_oled.c/h` + `bsp_oled_font.h` | SSD1315 I2C（PB6/PB7 400 kHz）；6×8 ASCII + 16×16 大数字字模；页级刷新 | ✅ 完成 |
...
| **CH6 继电器** | `bsp_relay.h: BSP_RELAY_CH6` | 枚举值已定义，自检不涉及，实际驱动管脚也已配置（PB15），业务层未使用 |
...
| **串口命令行（CLI）** | 中 | `App_Config_SetField` / `Dump` 接口已就绪；需实现接收解析模块 |
...
□ OLED 显示 "FloraMate V1.0 / BOOT" 启动画面
...
```
测试方法：进入菜单 → 手动测试页
操作：K1/K2 选阀 Z1~Z4 → K3 切换继电器状态
验证：
  □ 对应 CH 继电器可听见"咔哒"声
  □ OLED 显示阀号和 ON/OFF 状态
  □ 串口无 I2C 错误日志
退出时验证继电器全部 OFF（撤离手动测试页会调用 Deactivate_All）
```

#### 4.2.2 水泵 PWM

```
测试方法：手动测试页 → K2 切到 CH5 行 → K3 开关 → K1/K2 调 Duty
推荐步骤：
  1. K3 开 CH5（主阀）
  2. K4 +10%  →  K1 -5% 调节占空比（0~95%）
  3. 用示波器或逻辑分析仪测量 PA0 波形：
     □ 频率 1 kHz（±1%）
     □ 占空比与 OLED 显示一致
  4. K3 关 CH5，验证 PWM 立即归零
```
...
在 IDLE 页：
  □ 任意短按（< 300 ms）→ 进入菜单
  □ K1 长按（300 ms~3 s）→ 立即 Enter_AutoRun（跳过倒计时）
  □ 串口观察按键事件日志，确认无漏报 / 误报
  在 DONE/ERROR 页：
  □ K3 超长按（> 3 s）→ NVIC_SystemReset（系统重启）
```
...
步骤：
  ...
  2. 观察 AUTO_RUN 进度：
     ...
     □ CH1 继电器吸合（CH5 继电器吸合约 200 ms 后）
     □ PWM 按配置阶梯输出
  3. 单路完成后观察：
     □ PWM 先降到 0（RAMP_DOWN）→ CH5 断开 → CH1 断开 → 路间静默
     □ 继续下一路（若已使能）
  4. 全部完成后：
...
[503][I][FloraMate] [Set_State,...] sys fsm: -> 1   (SELFTEST)
[803][I][FloraMate] [Set_State,...] sys fsm: -> 2   (IDLE_3S)
[3803][I][FloraMate] [Enter_AutoRun,...] auto run: total 2 zones, starting Z1
...
```
...
5. 实现串口 CLI（命令行调试接口）
   - 接收格式：<cmd> [arg]\r\n
   - 建议命令：
     set <field> <value>  → App_Config_SetField
     get <field>          → App_Config_GetField
     dump                 → App_Config_Dump
     reset                → NVIC_SystemReset
     log <level>          → App_Log_SetLevel
     start                → Enter_AutoRun（测试用）
...
9. 暂停 / 继续功能
   - App_Pump_Fsm_Pause：保存当前 step_idx 和已用时间，把 PWM 降为 0 并断 CH5
   - App_Pump_Fsm_Resume：恢复 CH5 + 剩余时间继续
```
...
| **CH6 未使用** | 枚举已定义，管脚已配置（PB15），暂无业务连接 |
