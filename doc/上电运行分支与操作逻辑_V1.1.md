...
> **已废弃（Superseded）** — 自 **HMI V2.0**（2026-09-10）起，产品路径改为 Logo → Idle → 主界面 / 自动浇水。请参阅 [`上电运行分支与操作逻辑_V2.0.md`](上电运行分支与操作逻辑_V2.0.md) 与 [`屏幕与按键交互设计_V2.0.md`](屏幕与按键交互设计_V2.0.md)。下文仅作历史参考。

> V1.1 在 V1.0 基础上增加 **自动 / 手动** 双架构、K1+K2 进手动、手动子模式选择、串口退出回等待窗、按键手动控制。  
> **硬件（V3.0）**：1.3 寸 OLED；独立按键 K1=PA6/K2=PA7/K3=PB0/K4=PB1（按下低电平）；**AO4407A P-MOS 高边**驱动 **5 路阀 Z1~Z5（PA1~PA5）+ 水泵（PA0 PWM）**，高电平有效（与 `bsp_valve.h`、串口 `valve` 编号一致）。无机械继电器、无独立水泵电源通道。
...
无互锁，使用 `Bsp_Valve_DebugSet`；适合板级联调。

### 5.1 控制对象（光标 6 项，OLED 双列）

| 光标 | OLED | 通道 |
|------|------|--------|
| 0~4 | V1~V5 | 阀 Z1~Z5（PA1~PA5） |
| 5 | Pump | 水泵 PWM（PA0） |
...
| K3 | 阀开/关切换；Pump 项 **进入编辑**（再用 K1/K2 调 ±5%，K4 退出编辑停泵） |
